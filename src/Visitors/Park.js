...
class VisitorsPark extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
		  park: ''
	...
	componentDidMount() {
		this.fetchPark(this.props.match.params.id)
	}

  fetchPark(id) {
    axios.get(`https://localhost:5001/api/parks/${id}`)
				 .then(response => {
					 this.setState({park: response.data})
	...
	render() {
      ...
      <Row>
        <Col>
          <h3>{park.name}</h3>
          <p>{park.address} - {park.city}</p>
        </Col>
      </Row>
      <Row>
        <Col>
          <img width={800} src={park.image} />
        </Col>
      </Row>
      <Row>
      ...
      <Row>
				<Col>
					<Button onClick={() => {
						...
						let found = list.find(x => x.type === 'park' && x.name === park.name)
						if (!found) {
							list.push({type: 'Park', name: park.name, city: park.city, address: park.address})
						}
