...
class VisitorsShopping extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
		  shopping: ''
  ...
  fetchCave(id) {
    axios.get(`https://localhost:5001/api/shoppings/${id}`)
				 .then(response => {
	...
	render() {
      ...
      <Row>
        <Col>
          <h3>{shopping.name}</h3>
          <p>{shopping.address} - {shopping.city}</p>
        </Col>
      </Row>
      <Row>
        <Col>
          <img width={800} src={shopping.image} />
        </Col>
      </Row>
			<Row>
			...
			<Row>
				<Col>
					<Button onClick={() => {
						...
						let found = list.find(x => x.type === 'shopping' && x.name === shopping.name)
						if (!found) {
							list.push({type: 'Shopping', name: shopping.name, city: shopping.city, address: shopping.address})
						}
...
export default VisitorsShopping;
